{
    ...
    NameToPlural(config):: (
        if std.objectHas(config, 'plural') then
            ...
            local lname = std.asciiLower(config.kind);
  ...
  GetVersion(crd):: (
    local fv = [v for v in crd.spec.versions if 'name' in v];
    assert std.length(fv) == 1 : 'Could not find CRD with version';
    fv[0]
  ),
  ...
  GenCommonLabels(commonLabels):: (
    {
      [if std.length(commonLabels) > 0 then 'labels']: {
        ...
        for label in std.objectFields(commonLabels)
      },
    }
  ),
}
